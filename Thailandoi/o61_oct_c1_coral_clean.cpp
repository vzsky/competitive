...
void update(int now){
    // cerr << "now " << now << endl;
    seg[now] = 1;
    if ((int)log2(now) == k) leafcnt++; // leaf 
    if (now != 1) {
        // not root
        if (seg[now] && seg[now^1] && !seg[now/2]) update(now/2);
        if (seg[now] && seg[now/2] && !seg[now^1]) update(now^1);
    }
    if (seg[now] && seg[now*2] && !seg[now*2+1]) update(now*2+1);
    if (seg[now] && seg[now*2+1] && !seg[now*2]) update(now*2);
}
